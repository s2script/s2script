...
- Implementation: `7a212a9` (`fix(core): track only active plugin resources`).
...
- Full macOS core suite: 657 passed, 0 failed in 6.84s with the documented test-only linker wrapper;
  the same 10 warnings as the 646-test baseline remain. `scripts/check-core-boundary.sh` passes.
  Full output: `/tmp/s2script-hardening-slice1-final-macos.log`.
