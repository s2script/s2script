...
> - **STILL OPEN (finish offline, then live):** `sizeof(CUserCmd)` stride (protoc-seed per SHOULD-FIX 1, confirm via the loop-body `cmds[i]` indexing), the return type in RAX at the epilogue (forward it regardless), and the subtick verdict (needs a live human). The read-proof (Step 4) + modify/subtick (Step 6) require a human client on the server.
