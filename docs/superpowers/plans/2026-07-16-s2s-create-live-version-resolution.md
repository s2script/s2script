...
**Architecture:** Two-PR Graphite stack off `main` (`f30c4cd`). PR 1 (bottom) is a mechanical `packages/cli` cleanup that also fixes a currently-red build test (so PR 2 sits on a green base). PR 2 (top) is the version-resolution fix in the CLI plus a changeset. The version logic splits a thin `spawnSync` network seam (`resolvePublishedVersion`) from a pure, unit-testable formatter (`versionSpecFrom`), and threads an injectable resolver through `registryDevDeps` so tests never touch the network.
