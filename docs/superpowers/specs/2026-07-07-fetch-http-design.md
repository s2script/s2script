...
1. **Core — `core/src/http.rs`.** A **`tokio` multi-threaded runtime** started once at core init on a small fixed set of background threads (~4 — async I/O multiplexes thousands of connections over few threads via `epoll`; **not** thread-per-request), plus **one shared `reqwest::Client`** (rustls TLS, connection pooling + keep-alive) reused across all fetches. The runtime and the client are internal — invisible to plugins; the main game thread never runs them. **Lifecycle:** the engine is **process-global** in a `OnceLock`, exactly like the existing `async_rt` `POOL` — built once, never dropped (so it survives a Metamod re-init, same as the pool). On `shutdown()` the **fetch-pending map is cleared** alongside `RESOLVERS`, and any completions already queued are drained-and-dropped; an in-flight request that finishes *after* its owning plugin unloaded/reloaded (or after a re-init) lands with no pending entry / fails the liveness guard and is **dropped, never resolved into a dead context** — exactly the timer/job discipline. In-flight requests aren't actively cancelled (they complete off-thread and their results are harmlessly discarded), matching how the pool treats a late `threadSleep`.
