...
> **Proposed successor stack (2026-09-22):** The [shared engine bindings design](2026-09-22-engine-bindings-design.md) combines this document's remaining PR B/C backend work into S1, followed by simpler function authoring (S2) and game-package extraction (S3). On written-design approval, use that stack's dispatch boundaries while retaining this document's invocation/evidence requirements. PR A acceptance is still a prerequisite. The successor requires precache through stock KHook; retaining a private slot patch does not complete S1. These are design changes, not implementation completion.
...
1. **Process:** KHook. At pin `1e200e4`, `SaveReturnValue` accepts a **strictly higher** action: `Ignore < Override < Supersede`. The first executed return wins a tie, but a later `Supersede` replaces an earlier `Override`. Execution order must not be inferred from registration order: for the uniform `Function` peers in the acceptance fixture, the pinned insertion code places the newly registered wrapper before the previous one. Record actual callback order when testing equal-action precedence. All PRE callbacks still run. POST callbacks can affect the eventual return too; our POST observes the current effective result at its place in that chain, not a result frozen against later peer callbacks. See [the pinned implementation](https://github.com/Kenzzer/KHook/blob/1e200e4cc8e0badcb7cf941525268d6977f6a4e6/src/detour.cpp#L365), which is more precise than the README's first-wins shorthand.
