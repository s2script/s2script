...
The main new capability is **vtable-by-name resolution** (`shim/src/`): find `CNavPhysicsInterface`'s vtable in `libserver.so` by its **mangled symbol** (`_ZTVNN...` — exact name from `nm`/`readelf` in the spike) via `dlsym`, with an ELF `.symtab`/`.dynsym`-walk fallback (`dl_iterate_phdr`, already used for `FindModuleText`). This **self-resolves the vtable in our own binary** (doctrine-compliant); a reusable helper (`GetVTableByName(module, mangledSymbol)`) future SDKTools slices reuse.
