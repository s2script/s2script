...
Ships as **PR 2 (top) of a 2-PR Graphite stack**, both branched off current `main` (`f30c4cd`):

- **PR 1 (bottom) — `packages/cli` cleanup** (separate, mechanical). The CLI-into-`@s2script/sdk` consolidation moved every test fixture into `packages/sdk/test/fixtures/` *except* `publisher-mapform-typeerror`, whose only copy is still under the otherwise-dead `packages/cli/`. That strands a live test: `packages/sdk/test/build.test.mjs`'s "build rejects a RANGE BEFORE the typecheck gate" resolves the fixture via `join(here, "fixtures", …)` (now the sdk dir), hits `ENOENT`, and fails — a real regression, distinct from the 13 known `schema-runtime`/`player-identity` failures. PR 1: migrate the 3 stranded fixture files into `packages/sdk/test/fixtures/` (turns that test green), delete the dead `packages/cli/` shell, and prune the stale `@s2script/cli` entries from `package-lock.json` (it still records `packages/cli` → `@s2script/cli@0.2.0`). No behavior change to shipped code.
- **PR 2 (top) — this version-resolution fix**: `packages/sdk/src/create/create.ts` + its test file, plus a **changeset** (patch bump for `@s2script/sdk`).

Cleanup goes on the bottom so PR 2 sits on a green test base. Run the gate suite **per PR** (each must pass on its own); `./scripts/check-plugins-typecheck.sh` is the relevant CLI-adjacent gate, and `npm test` in `packages/sdk` must be green after PR 1's fixture migration.
