...
**RESOLVED (2026-07-14, live human spike): coarse writes SUFFICE → Option B.** A key-gated spike proved that zeroing `forwardmove` on the coarse `base` field ALONE stops the player (holding E = coarse-only, no subtick clear, brought them to a stop). So `subtick_moves` do NOT override the coarse fields on write. The movement write ops (`s2_usercmd_write` fields 0/1/2) just set the coarse field with **NO auto-subtick-clear**; `clearSubtickMoves()` is exposed as an OPTIONAL helper (not part of the write contract). Also confirmed: `forwardMove`/`sideMove` are **normalized to ±1** in the usercmd (not the old ±450), and reflection reads/writes `base.forwardmove` + `base.buttons_pb.buttonstate1` correctly.
