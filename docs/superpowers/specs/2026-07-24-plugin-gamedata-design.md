...
Allowed validators (evaluated shim-side, after `.text`-range):

| Validator | Meaning |
|---|---|
| `prologue` | masked byte pattern (`??` wildcards) the resolved function must begin with |
| `stringRef` | the function body must reference this string constant |

`prologue` is what catches the thunk case: a thunk's prologue does not match the real function's.
...
- **Max 6 args.** All register-passed under SysV; stack-argument marshalling is deferred.
- Calling convention: C++ member function — `this` first (rdi), integers in GP registers, floats in
  xmm. Overloads and variadics are out of scope.
...
duration of the call; `vector` args into a stack temp passed by address. This is what keeps the
feature reviewable and inside the existing safety model.
...
2. **Operator allow-list, default-deny.** `addons/s2script/configs/permissions.json` lists plugin ids
   permitted to use `engine:calls`, mirroring the admin system's fail-safe default-deny. It ships as
...
|---|---|---|
| Build | `vtable` without `validate`; unknown arg/return type; >6 args; `calls` without `engine:calls`; malformed JSON | **build fails** — no `.s2sp` |
...
- Stack-passed args beyond 6; overloads; variadics.
