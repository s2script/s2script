...
- **Type mapping → the existing `SqlValue` union** (`string | number | boolean | null`): integers that fit f64 → `number`; **`BIGINT`/`DECIMAL`/`NUMERIC` → decimal `string`** (precision — steamids as BIGINT; the 5B.4 "64-bit as decimal string" rule; `string` is already in the union); float/double → `number`; text/varchar/char → `string`; bool → `boolean`; NULL → `null`; DATE/TIME/TIMESTAMP → ISO `string`; BLOB/`bytea` → **deferred** (→ `null`, as the SQLite driver already skips blobs). Params (`SqlValue` in): string → text bind, number → int/float bind, bool → bool, null → null.
