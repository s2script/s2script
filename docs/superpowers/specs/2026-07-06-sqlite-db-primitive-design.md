...
- Each operation submits a closure to the **`async_rt` threadpool**; the closure locks the connection's `Mutex`, runs the rusqlite call, and enqueues a **frame-drain resolver** that settles the Promise. The per-connection `Mutex` serializes queries on one connection (SQLite is single-thread-at-a-time anyway).
