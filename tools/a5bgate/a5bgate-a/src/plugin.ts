...
export function OnPluginStart(): void {
  ...
  command.server("a5b_dupe_respawn", () => {
    const dead = Player.all().find((p) => (p.pawn?.health ?? 1) <= 0) ?? anyPlayer();
