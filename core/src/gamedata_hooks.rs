...
pub(crate) const SHAPES: &[(&str, i32)] = &[
    ...
    ("this_i64_i32_i64", 3),
    // void(this, i64, i64, i64) — three FULL-WIDTH pointer args. Added for
    // CCSCustomHudLayout's HUD-click receiver, whose args are (pulse binding, CCSPlayerController*,
    // CCSCustomHudLayout*, const std::string* buttonId). Every one is a pointer, so there is no
    // shape above that can carry it: `this_i64_i32_i64`'s middle slot is i32, and narrowing a
    // pointer is the exact bug documented in shim/src/hook_dispatch.h that segfaulted a live
    // server. See that comment before ever reaching for a narrower shape again.
    ("this_i64_i64_i64", 4),
];
