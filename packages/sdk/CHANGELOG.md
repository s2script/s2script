...
- dd8f333: Plugins can declare inbound engine hooks in their own gamedata.
...
- 2e7eb80: Let `WebSocket.connect` send headers on the opening handshake
...
- f8df0d8: Add `EntityRef.origin`, reading `CGameSceneNode::m_vecAbsOrigin`
...
- 1e3e691: Add `EntityRef.target`, reading `CBaseEntity::m_target`
...
- d91add2: Fix `s2s build`'s plugin-gamedata naming-mismatch error message. It used to say a misnamed
  ...
  itself), the same way `gamedata/core/` and `gamedata/cs2/` are owner-named _directories_ in the
...
- 3986f43: Add SourceMod's six custom admin flags (`ADMFLAG.CUSTOM1`-`CUSTOM6`) to the admin flag model
