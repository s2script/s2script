...
| [`interop-observer`](examples/interop-observer) | An optional protocol 2 consumer that watches BaseComm mute/gag policy and BaseBans operations, owns per-provider subscriptions, and builds from a verified types-only contract copy. |
| [`interop-acceptance`](tools/interop-acceptance) | Four opt-in test plugins: same-name notifications, hooks/transforms, named aliases, service probes and deferred provider reload acceptance. |
