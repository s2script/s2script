...
static void test_shape_vocabulary_is_closed() {
    ...
    // The retired pickup-gate / HUD-click shapes fail BY NAME and by id; their ids are not reused.
    CHECK(S2Hook_ShapeFromName("this_i64_i32_i64") == -1, "the retired returning shape is gone");
    CHECK(S2Hook_ShapeFromName("this_i64_i64_i64") == -1, "the retired HUD-click shape is gone");
    CHECK(S2Hook_ShapeName(3) == nullptr && S2Hook_ShapeName(4) == nullptr,
          "retired shape ids have no name");
}
