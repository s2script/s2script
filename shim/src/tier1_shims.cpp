...
// ---------------------------------------------------------------------------------------------------
// EKV slice (compile-spike, Task 1) — leaf self-shims for entity2/entitykeyvalues.cpp +
// tier1/keyvalues3.cpp (the SDK's own CEntityKeyValues/KeyValues3 sources, compiled into the shim
// per the CSSharp approach). Classified via the plan's Rule 1 (self-contained logging/string leaves)
// and confirmed via `nm -Cu` triage against a Task-1 sniper build + `readelf -Wr` reachability tracing
// (which caller in the compiled .o actually references each symbol) — every symbol below is either a
// pure string/number-parsing leaf or a discard-the-message logging no-op, none holds engine state or
// needs a live interface. All PLATFORM_INTERFACE-declared in tier0/dbg.h, tier0/logging.h, or
// tier0/platform.h with `extern "C"` linkage (DLL_EXPORT expands to `extern "C"
// __attribute__((visibility("default")))`), hence unmangled names below; MurmurHash2 is the one
// exception (regular C++ linkage, no PLATFORM_INTERFACE) — reuses the s2_MurmurHash2 core above
// (Valve's MurmurHash2 and MurmurHash2LowerCase share the identical core loop; this is the
// non-lowercased sibling CUtlSymbolLarge_Hash's `false` branch calls).
//
// Per the self-shim doctrine (prefer these over live-resolution even where tier0 exports some of
// them) — verified live: LOG_GENERAL/LoggingSystem_Log/LoggingSystem_IsChannelEnabled ARE exported by
// the container's libtier0.so, but Warning/Plat_ExitProcess/Plat_NonFatalErrorFunc/MurmurHash2/
// V_StringToBool/V_StringToFloat32/V_StringToInt32/_V_strncpy/V_tier0_memmove are NOT exported by
// libtier0.so or libserver.so on this build — self-shimming all of them uniformly avoids depending on
// which subset Valve happens to export this patch (the 2000870 lesson).
// ---------------------------------------------------------------------------------------------------
#include <cstdarg>
#include <cstdio>
