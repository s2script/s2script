...
- **Layout is data, semantics are code.** Offsets/signatures/struct positions/interface strings live in regenerable `gamedata`/schema files; name mappings in reviewed code. Core gamedata carries layout only. Game-package and plugin gamedata may carry per-game behavioural strings (`keys`) — asset paths, sounds, colours, tunables — because those are per-game data, not logic. Decision logic stays in reviewed code. A field-offset change must never require a code change.
- **Resolve every engine fact with the weakest resolver that still identifies it, not the shortest** (Bennett's Razor: *explanations should be no more specific than necessary*). Each fact is inferred from the one build in front of us and must hold on builds we've never seen, so rank the candidates that all resolve correctly *today* by how many future builds they'd also survive — baked offset < pinned byte pattern < wildcarded pattern < string-xref < RTTI/schema name lookup — and take the weakest even though it is always the longest code. **A repair must be weaker than what it broke:** re-deriving a constant that moved just reschedules the same failure. Full doctrine + the ordering table: `docs/re-strategy.md`.
