...
export function OnPluginStart(): void {
  ...
  for (const name of ["active", "slot", "pawn"]) {
  ...
  command("s2_engine_accept", cmd => {
    ...
    if (op === "arm") {
      ...
      try {
        ...
        record("acquire-stimulus", { botSlot: bot.slot, pawn: pawn.ref.index, itemCreated: item !== null });
        if (item) pawn.removeWeapon(item);
      } finally { Server.setCvar("s2_engine_fixture_active", "-1"); }
  ...
  items.onCanAcquirePost(view => {
    if (!run || Number(Server.getCvar("s2_engine_fixture_active")) !== generation || view.player?.slot !== Number(Server.getCvar("s2_engine_fixture_slot"))) return;
    record("acquire-post", { slot: view.player?.slot ?? -1, defIndex: view.defIndex, result: view.result, skipped: view.skipped });
  });
  record("loaded");
}
