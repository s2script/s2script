...
```
gamedata/engine-call.gamedata.jsonc   what to call, and how it was found on the old build
...
Your gamedata is your treadmill. The shipped Ignite pattern has already stopped matching the audited
current image, so that call degrades with a named reason. Re-deriving it is your job, not the
framework's; this descriptor also needs its omitted native `Vector` ABI addressed.
