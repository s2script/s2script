 ...
 *
 *   cb_transmit          spawn a labelled point_worldtext at the first human's
 *                        feet (so it is genuinely in their PVS) and bind that slot
 *   cb_transmit_hide     hide it from a slot (default: the bound human) — visible to everyone else
 *   cb_transmit_only     show it ONLY to a slot (default: the bound human)
 *   cb_transmit_show     reset the rule (visible to all)
 *   cb_transmit_stats    print Transmit.stats()
 */
export const transmitRecipe: Recipe = {
  name: "transmit",
  describe: "per-client entity visibility rules (cb_transmit / _hide / _only / _show / _stats)",
  register(ctx) {
    let prop: EntityRef | null = null;
    let boundSlot = -1;

    ctx.commands.register("cb_transmit", (cmd) => {
    ...
    ctx.commands.register("cb_transmit_hide", (cmd) => {
      if (!prop || !prop.isValid()) { cmd.reply("[cookbook] transmit: no live prop — cb_transmit first"); return; }
      const slot = cmd.argInt(0, boundSlot);
      if (slot < 0 || slot >= 64) { cmd.reply("[cookbook] usage: cb_transmit_hide <slot 0-63>"); return; }
      const ok = Transmit.setVisibleTo(prop, ALL_SLOTS.filter((s) => s !== slot));
      cmd.reply("[cookbook] transmit: hide from slot " + slot + " -> " + ok);
    });

    ctx.commands.register("cb_transmit_only", (cmd) => {
      if (!prop || !prop.isValid()) { cmd.reply("[cookbook] transmit: no live prop — cb_transmit first"); return; }
      const slot = cmd.argInt(0, boundSlot);
      if (slot < 0 || slot >= 64) { cmd.reply("[cookbook] usage: cb_transmit_only <slot 0-63>"); return; }
      const ok = Transmit.setVisibleTo(prop, [slot]);
      cmd.reply("[cookbook] transmit: visible ONLY to slot " + slot + " -> " + ok);
    });

    ctx.commands.register("cb_transmit_show", (cmd) => {
      if (!prop) { cmd.reply("[cookbook] transmit: no prop"); return; }
      cmd.reply("[cookbook] transmit: reset -> " + Transmit.reset(prop));
    });

    ctx.commands.register("cb_transmit_stats", (cmd) => {
