...
import { pawnRef, pawnHealth } from "@demo/ent"; // hard dep -> producer-backed proxy
...
export function onLoad(): void {
  ...
  OnGameFrame.subscribe(() => {
    ...
    try {
      const ref = pawnRef(0);                            // a LIVE EntityRef received across the wire
      ...
      console.log("[consumer] tick " + ticks + " received-ref valid=" + alive
        + " health=" + (alive ? pawnHealth(0) : "null"));
